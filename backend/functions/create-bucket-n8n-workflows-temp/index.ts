...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/json", "text/plain"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for n8n-workflows" ON storage.objects FOR SELECT USING (bucket_id = 'n8n-workflows');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for n8n-workflows" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'n8n-workflows');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for n8n-workflows" ON storage.objects FOR UPDATE USING (bucket_id = 'n8n-workflows');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for n8n-workflows" ON storage.objects FOR DELETE USING (bucket_id = 'n8n-workflows');`
    ];
