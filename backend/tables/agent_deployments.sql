...
COMMENT ON TABLE agent_deployments IS 'Manages deployed AI agents generated from n8n workflows';
COMMENT ON COLUMN agent_deployments.deployment_type IS 'Type of deployment: container, serverless, or edge';
COMMENT ON COLUMN agent_deployments.instance_config IS 'Instance-specific configuration like CPU, memory limits';
COMMENT ON COLUMN agent_deployments.resource_limits IS 'Resource limits and quotas for the deployed agent';
COMMENT ON COLUMN agent_deployments.deployment_logs IS 'Deployment process logs and status messages';
COMMENT ON COLUMN agent_deployments.total_cost IS 'Total cost incurred by this agent in ooumph coins';
