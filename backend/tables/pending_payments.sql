...
COMMENT ON TABLE pending_payments IS 'Tracks Stripe payment intents for ooumph coin purchases';
COMMENT ON COLUMN pending_payments.stripe_payment_intent_id IS 'Stripe Payment Intent ID for tracking';
COMMENT ON COLUMN pending_payments.credit_amount IS 'Number of ooumph coins to be purchased';
COMMENT ON COLUMN pending_payments.fiat_amount IS 'Amount charged in fiat currency (with 25% margin)';
COMMENT ON COLUMN pending_payments.stripe_data IS 'Full Stripe webhook data for debugging';
