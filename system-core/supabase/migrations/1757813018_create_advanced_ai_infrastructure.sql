-- Migration: create_advanced_ai_infrastructure
-- Created at: 1757813018

-- Migration: Advanced AI Features - Phase 1 Core Infrastructure
-- Created at: 1757811900
...
CREATE POLICY "Security admins can manage threat detection" ON security_threat_detection
    FOR ALL TO authenticated USING (
        auth.jwt() ->> 'email' IN (
            SELECT email FROM profiles WHERE role IN ('admin', 'security_admin')
        )
    );
