-- Migration: create_ai_training_infrastructure
-- Created at: 1757801538

-- Migration: AI Training & Learning System Infrastructure
-- Created at: 1757798000
...
CREATE TABLE IF NOT EXISTS ai_training_datasets (
    ...
    data_lineage JSONB DEFAULT '{}', -- Track data sources and transformations
...
CREATE TABLE IF NOT EXISTS training_data_samples (
    ...
    sample_index INTEGER NOT NULL, -- Order within dataset
    split_type TEXT NOT NULL CHECK (split_type IN ('train', 'validation', 'test')),
    input_data JSONB NOT NULL,
    expected_output JSONB NOT NULL,
    sample_metadata JSONB DEFAULT '{}',
    quality_indicators JSONB DEFAULT '{}', -- Bias, toxicity, factuality scores
...
CREATE TABLE IF NOT EXISTS data_quality_metrics (
    ...
    evaluation_method TEXT, -- human, automated, hybrid
...
CREATE TABLE IF NOT EXISTS ai_training_jobs (
    ...
    training_objective TEXT NOT NULL, -- What the model should learn to do better
    ...
    resource_usage JSONB DEFAULT '{}', -- Compute time, memory, cost
...
CREATE TABLE IF NOT EXISTS ai_model_versions (
    ...
    version_number TEXT NOT NULL, -- e.g., v1.0, v1.1, etc.
    base_model TEXT NOT NULL,
    specialization_domain TEXT NOT NULL,
    model_size TEXT, -- small, medium, large, xlarge
    performance_metrics JSONB DEFAULT '{}', -- Benchmark scores
    deployment_config JSONB DEFAULT '{}',
    model_artifacts_path TEXT, -- Storage location for model files
    status TEXT DEFAULT 'training' CHECK (status IN (
        'training', 'ready', 'deployed', 'deprecated', 'archived'
    )),
    deployment_url TEXT, -- API endpoint for deployed model
    resource_requirements JSONB DEFAULT '{}', -- Memory, compute needs
    created_by UUID REFERENCES auth.users(id),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
...
CREATE TABLE IF NOT EXISTS training_metrics_timeline (
    ...
    metrics JSONB NOT NULL, -- loss, accuracy, perplexity, etc.
...
CREATE TABLE IF NOT EXISTS specialized_ai_agents (
    ...
    capabilities JSONB NOT NULL, -- Detailed capability matrix
    performance_benchmarks JSONB DEFAULT '{}', -- Benchmark test results
    ...
    api_endpoint TEXT, -- Direct access endpoint
...
CREATE TABLE IF NOT EXISTS agent_performance_benchmarks (
    ...
    comparison_baseline DECIMAL(5,4), -- Score of baseline model
...
CREATE TABLE IF NOT EXISTS user_feedback_logs (
    ...
    interaction_context TEXT NOT NULL, -- What task was being performed
    ...
    user_input JSONB, -- Original user request
    agent_response JSONB, -- Agent's response
    preferred_response JSONB, -- User's preferred response (if provided)
    response_quality_aspects JSONB DEFAULT '{}', -- accuracy, helpfulness, clarity, etc.
    learning_insights JSONB DEFAULT '{}', -- Extracted patterns for improvement
...
CREATE TABLE IF NOT EXISTS training_hyperparameters (
    ...
    effectiveness_score DECIMAL(3,2), -- Overall effectiveness rating
    notes TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);
...
CREATE TABLE IF NOT EXISTS training_analytics (
    ...
    aggregation_method TEXT, -- sum, average, max, min, count
    context_filters JSONB DEFAULT '{}', -- Filters applied to calculate metric
...
CREATE TABLE IF NOT EXISTS agent_comparison_results (
    ...
    winner_agent_id UUID, -- NULL for ties
...
-- RLS Policies - Allow authenticated users to view, admins/creators to modify

-- Training Datasets - Users can view public datasets, modify their own
CREATE POLICY "Allow users to view public training datasets" ON ai_training_datasets
    FOR SELECT TO authenticated 
    USING (privacy_level = 'public' OR created_by = auth.uid());

CREATE POLICY "Allow users to manage their own datasets" ON ai_training_datasets
    FOR ALL TO authenticated 
    USING (created_by = auth.uid());

-- Training Data Samples - Follow dataset permissions
CREATE POLICY "Allow access to training samples based on dataset" ON training_data_samples
    FOR SELECT TO authenticated 
    USING (EXISTS (
        SELECT 1 FROM ai_training_datasets 
        WHERE id = dataset_id 
        AND (privacy_level = 'public' OR created_by = auth.uid())
    ));

-- Training Jobs - Users can view and manage their own jobs
CREATE POLICY "Allow users to manage their training jobs" ON ai_training_jobs
    FOR ALL TO authenticated 
    USING (created_by = auth.uid());

-- Specialized Agents - Public agents viewable by all, private by creators
CREATE POLICY "Allow viewing specialized agents" ON specialized_ai_agents
    FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow creators to manage agents" ON specialized_ai_agents
    FOR ALL TO authenticated 
    USING (created_by = auth.uid());

-- User Feedback - Users can only see their own feedback
CREATE POLICY "Allow users to manage their own feedback" ON user_feedback_logs
    FOR ALL TO authenticated 
    USING (user_id = auth.uid());

-- Analytics - Public read access for authenticated users
CREATE POLICY "Allow authenticated users to view analytics" ON training_analytics
    FOR SELECT TO authenticated USING (true);

-- Apply similar policies to other tables
CREATE POLICY "Allow authenticated users to view data quality metrics" ON data_quality_metrics
    FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow authenticated users to view model versions" ON ai_model_versions
    FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow authenticated users to view training metrics" ON training_metrics_timeline
    FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow authenticated users to view agent specializations" ON agent_specializations
    FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow authenticated users to view benchmarks" ON agent_performance_benchmarks
    FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow authenticated users to view optimization logs" ON performance_optimization_logs
    FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow authenticated users to view hyperparameters" ON training_hyperparameters
    FOR SELECT TO authenticated USING (true);

CREATE POLICY "Allow authenticated users to view comparisons" ON agent_comparison_results
    FOR SELECT TO authenticated USING (true);;