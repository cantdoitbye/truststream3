-- Migration: create_economic_ai_layer
-- Created at: 1758257353

-- Economic AI Layer with DAO Formation and TrustCoin Integration
...
CREATE TABLE trust_scores (
    ...
    -- Constraints
    CONSTRAINT trust_scores_score_range CHECK (current_score >= 0 AND current_score <= 1000),
    CONSTRAINT trust_scores_agent_unique UNIQUE (agent_id)
);

-- TrustCoin Transactions Table
-- Tracks all TrustCoin transactions including minting, transfers, rewards, and penalties
CREATE TABLE trust_coin_transactions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE dao_members (
    ...
    -- Constraints
    CONSTRAINT dao_members_agent_unique UNIQUE (agent_id),
    CONSTRAINT dao_members_valid_reputation CHECK (reputation_level IN ('bronze', 'silver', 'gold', 'platinum', 'diamond')),
    CONSTRAINT dao_members_positive_voting_power CHECK (voting_power >= 0),
    CONSTRAINT dao_members_positive_stake CHECK (stake_amount >= 0),
    CONSTRAINT dao_members_valid_participation CHECK (governance_participation_score >= 0 AND governance_participation_score <= 100)
);

-- DAO Proposals Table
-- Manages governance proposals within the DAO
CREATE TABLE dao_proposals (
    ...
    -- Constraints
    CONSTRAINT dao_proposals_valid_type CHECK (proposal_type IN (
        'parameter_change', 'fund_allocation', 'member_addition', 'member_removal', 
        'treasury_management', 'system_upgrade', 'partnership', 'other'
    )),
    CONSTRAINT dao_proposals_valid_status CHECK (status IN (
        'draft', 'active', 'passed', 'rejected', 'expired', 'executed', 'cancelled'
    )),
    CONSTRAINT dao_proposals_valid_voting_period CHECK (voting_end > voting_start),
    CONSTRAINT dao_proposals_valid_quorum CHECK (required_quorum >= 0 AND required_quorum <= 100),
    CONSTRAINT dao_proposals_valid_majority CHECK (required_majority >= 0 AND required_majority <= 100)
);
...
CREATE TABLE dao_votes (
    ...
    -- Constraints
    CONSTRAINT dao_votes_valid_choice CHECK (vote_choice IN ('for', 'against', 'abstain')),
    CONSTRAINT dao_votes_positive_power CHECK (voting_power_used > 0),
    CONSTRAINT dao_votes_unique_vote UNIQUE (proposal_id, voter_id)
);
...
CREATE INDEX idx_trust_scores_current_score ON trust_scores(current_score DESC);
CREATE INDEX idx_trust_scores_last_calculated ON trust_scores(last_calculated_at);

CREATE INDEX idx_trust_coin_from_agent ON trust_coin_transactions(from_agent_id);
CREATE INDEX idx_trust_coin_to_agent ON trust_coin_transactions(to_agent_id);
CREATE INDEX idx_trust_coin_type ON trust_coin_transactions(transaction_type);
CREATE INDEX idx_trust_coin_status ON trust_coin_transactions(status);
CREATE INDEX idx_trust_coin_created_at ON trust_coin_transactions(created_at);

CREATE INDEX idx_dao_members_agent_id ON dao_members(agent_id);
CREATE INDEX idx_dao_members_active ON dao_members(is_active);
CREATE INDEX idx_dao_members_reputation ON dao_members(reputation_level);
CREATE INDEX idx_dao_members_voting_power ON dao_members(voting_power DESC);
...
CREATE INDEX idx_dao_proposals_voting_period ON dao_proposals(voting_start, voting_end);

CREATE INDEX idx_dao_votes_proposal ON dao_votes(proposal_id);
CREATE INDEX idx_dao_votes_voter ON dao_votes(voter_id);
CREATE INDEX idx_dao_votes_choice ON dao_votes(vote_choice);

CREATE INDEX idx_regional_pricing_region ON regional_pricing_tiers(region_code);
CREATE INDEX idx_regional_pricing_service ON regional_pricing_tiers(service_type);
CREATE INDEX idx_regional_pricing_active ON regional_pricing_tiers(is_active);
CREATE INDEX idx_regional_pricing_effective ON regional_pricing_tiers(effective_from, effective_until);

CREATE INDEX idx_economic_analytics_type ON economic_analytics(metric_type);
CREATE INDEX idx_economic_analytics_recorded ON economic_analytics(recorded_at);
CREATE INDEX idx_economic_analytics_period ON economic_analytics(aggregation_period);
...
CREATE POLICY "Enable read access for all users" ON trust_scores FOR SELECT USING (true);
CREATE POLICY "Enable read access for all users" ON trust_coin_transactions FOR SELECT USING (true);
CREATE POLICY "Enable read access for all users" ON dao_members FOR SELECT USING (true);
CREATE POLICY "Enable read access for all users" ON dao_proposals FOR SELECT USING (true);
CREATE POLICY "Enable read access for all users" ON dao_votes FOR SELECT USING (true);
...
CREATE VIEW dao_governance_summary AS
SELECT 
...
CREATE VIEW proposal_statistics AS
SELECT 
    proposal_type,
    status,
    COUNT(*) as count,
...
SELECT 
    (SELECT COUNT(*) FROM dao_members WHERE is_active = true) as active_dao_members,
    (SELECT COUNT(*) FROM dao_proposals WHERE status = 'active') as active_proposals,
    (SELECT SUM(amount) FROM trust_coin_transactions WHERE transaction_type = 'mint' AND status = 'confirmed') as total_minted_coins,
    (SELECT COUNT(*) FROM trust_coin_transactions WHERE created_at >= NOW() - INTERVAL '24 hours') as daily_transactions,
    (SELECT AVG(current_score) FROM trust_scores) as avg_trust_score;

-- Comments for documentation
COMMENT ON TABLE trust_scores IS 'Reputation scores for agents based on behavior and performance';
COMMENT ON TABLE trust_coin_transactions IS 'All TrustCoin transactions including minting, transfers, rewards, and penalties';
COMMENT ON TABLE dao_members IS 'DAO membership and voting power for agents';
COMMENT ON TABLE dao_proposals IS 'Governance proposals within the DAO';
